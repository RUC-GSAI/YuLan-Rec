...
class ChatGLM(LLM):
    ...
    def _call(
        ...
        if response.status_code == 200:
            ...
            self.logger.error('ChatGLM error occurred with status code:', response.text)
        return response.text
    
    @property
    def _identifying_params(self) -> Mapping[str, Any]:
        """Get the identifying parameters."""
        return {"max_token": self.max_token,"URL":self.URL,"headers":self.headers,"payload":self.payload}